<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
  <meta name="description" content="">
  <link rel="stylesheet" type="text/css" href="css/base.css"/>
  <link rel="stylesheet" type="text/css" href="css/style.css"/>
  <link rel="stylesheet" type="text/css" href="css/iconfont/iconfont.css"/>
  <title>用户登录</title>
	</head>
	<body>
		<div class="header">
			<span>用户登录</span>
			<!--<a href="#" class="back"><i class="iconfont icon-left"></i></a>-->
		</div>
		<ul class="login-box">
			<li>
				    <span class="login-name"><i class="iconfont icon-user"></i></span>
		        <input type="text" id="name" name="name" placeholder="请输入用户名" />   
			</li>
			<li>
				<span class="login-psw"><i class="iconfont icon-psw"></i></span>
		        <input type="text" id="phone" name="phone" placeholder="请输入手机号码后六位" />   
			</li>
		</ul>
		</div>
		<div class="confirm">
			<a href="index1" id="submit" class="ta-center db">登录</a>
			<p class="foget-psw"><a href="#">忘记密码</a></p>
		</div>
	</body>
	<script src="js/jquery-1.10.2.min.js"></script>
     <script type="text/javascript" src="js/rem.js"></script>
     <script type="text/javascript">
			        $("#submit").click(function(){
			        	//个人信息
			            var name=$("#name").val();
			            var phone=$("#phone").val();
			            // console.log(111);
			            $.ajax({
			            	type: 'GET',
			            	url: "http://localhost/xiao/tpp/index.php/Home/login/index?callback=?",
			            	dataType: 'jsonp',
			            	callback:'callback',
			            	 data:{name:name,phone:phone},
			            	success: function(msg){
			            		// console.log(callback);
			            		alert(msg);			            	
			            	}
			            })
			            alert('成功');
			    })
			</script>	
</html>
